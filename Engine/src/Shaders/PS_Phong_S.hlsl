//Phong pixel shader with specular mapping.
...
float3 CalculateDirectionalLights(float3 normal, float3 viewSpacePosition, float3 specularColor, float shininess)
{
    ...
    return totalDiffuse * (float3) matDiffuseColor + totalSpecular * specularColor * matShininessStrength;
}

float3 CalculatePointLights(float3 normal, float3 viewSpacePosition, float3 specularColor, float shininess)
{
    float3 totalDiffuse = { 0.0f, 0.0f, 0.0f };
    float3 totalSpecular = { 0.0f, 0.0f, 0.0f };
